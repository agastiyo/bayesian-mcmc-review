...
Since we know nothing about the priors, it is reasonable to assume that they are uniformly distributed in a certain range. Looking at the data, we can see that the velocity semi-amplitude $K$ is between 0 and 20 m/s, the period $T$ is between 0 and 20 days, and the phase offset $\phi$ is between 0 and $2\pi$. Thus, the priors are:
...
\begin{equation}
  P(K,T,\phi) = \begin{cases}
    \frac{1}{800\pi} & 0 < K < 20, 0 < T < 20, 0 < \phi < 2\pi \\
    0 & \text{otherwise}
  \end{cases}
\end{equation}

Next, we must determine the likelihood, $P(D|K,T,\phi)$.
